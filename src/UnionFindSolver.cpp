...
void UnionFindSolver::unify(std::unique_ptr<Term> t1,std::unique_ptr<Term> t2)
{
    // makeSet(t1);
    // makeSet(t2);
...
std::unique_ptr<Term> UnionFindSolver::find(std::shared_ptr<Term> p)
{
    // makeSet(p);
    // auto copy = p;
    // while(parent[p] != p)
...
void UnionFindSolver::makeUnion(std::unique_ptr<Term> t1,std::unique_ptr<Term> t2)
{
    // parent[t1] = t2;
}

void UnionFindSolver::makeSet(std::unique_ptr<Term> t)
{
...
std::unordered_map<std::unique_ptr<Var>,std::unique_ptr<Term>> UnionFindSolver::solution()
{
    // TODO
    return std::unordered_map<std::unique_ptr<Var>,std::unique_ptr<Term>>();
}